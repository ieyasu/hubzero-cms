		...
		<ul class="discussions">
			<?php foreach ($this->posts as $post) : ?>
				<?php if ($c < $this->limit) : ?>
					...
					?>
          <li class="featuredtoolitem"><a href="<?php echo JRoute::_($url); ?>" title=""><?php echo ($post['parent'] && isset($this->threads[$post['parent']])) ? stripslashes($this->threads[$post['parent']]) : stripslashes($post['title']); ?></a>
            <p class="small"><?php
                if ($post['anonymous']) {
                ...
                echo ', in&nbsp;';
                echo $location . ' ';
                echo date("F jS, Y, g:ia", strtotime($post['created'])); ?></p>
          </li>
				<?php endif; ?>
				<?php $c++; ?>
			<?php endforeach; ?>

...
Flotr.defaultOptions = {
	...
	xaxis: {
		...
		positionTop: false,      // => setting for if labels appear along the top of the chart or bottom
    ...
    **/
		...
		[x, x2, y, y2].each(function(axis) {
			...
			if (axis.options.showLabels) {
				for(i = 0; i < axis.ticks.length; ++i){
					if (axis.ticks[i].label.indexOf('\n') !=-1) {
						bits = axis.ticks[i].label.split('\n');
	...
	drawGrid: function(){
		var v, o = this.options,
		    ctx = this.ctx, a;
		/*if (this.axes.x.options.positionTop && this.axes.x.options.labelsAngle) {
			this.plotOffset.top += 15;
			this.plotHeight -= 15;
		}*/
   ...
   */
	...
	drawLabels: function(){		
		...
		if (!options.HtmlText && this.textEnabled) {
			...
			//for(i = 0; i < axis.ticks.length && axis.options.showLabels && axis.used; ++i){
			ofst = 0;
			if (a.x.options.positionTop) {
				ofst = a.x.d2p(axis.ticks[1].v)/2;
			}
			for(i = 0; i < axis.ticks.length && axis.options.showLabels; ++i){
				tick = axis.ticks[i];
				if(!tick.label || tick.label.length == 0) continue;
				...
				ctx.drawText(
					tick.label,
					this.plotOffset.left + left - ofst, 
			...
			axis = a.y;
			style.color = axis.options.color || options.grid.color;
			ofst = (axis.d2p(axis.ticks[0].v) - axis.d2p(axis.ticks[1].v))/2;
			for(i = 0; i < axis.ticks.length && axis.options.showLabels && axis.used; ++i){
				tick = axis.ticks[i];
				if (!tick.label || tick.label.length == 0) continue;
				...
				//CanvasText.debug = true;
				if (tick.label.indexOf('\n') !=-1) {
					bits = tick.label.split('\n');
					for(z = 0; z < bits.length; ++z){
						ofs = ofst;
						if (z > 0) {
							var dim = this.getTextDimensions(bits[z], null, null);
							ofs += dim.height;
						}
						ctx.drawText(
							bits[z],
							this.plotOffset.left - options.grid.labelMargin, 
							this.plotOffset.top + top + ofs,
							style
						);
					}
				} else {
			...
			if (axis.options.showLabels){
				for(i = 0; i < axis.ticks.length; ++i){
					...
					if (a.x.options.positionTop) {
						if (a.x.options.labelsAngle) {
							if (a.x.options.labelsAngle == 90) {
								html.push('<div style="position:absolute;top:-'+(xBoxWidth/3)+'px;left:' + (this.plotOffset.left +axis.d2p(tick.v) - xBoxWidth/2) + 'px;width:' + xBoxWidth + 'px;text-align:left;'+(axis.options.color?('color:'+axis.options.color+';'):'')+'" class="flotr-grid-label x-axis">' + tick.label + '</div>');
							} else {
								html.push('<div style="position:absolute;top:-'+(xBoxWidth)+'px;left:' + ((this.plotOffset.left + axis.d2p(tick.v) - xBoxWidth/2) - options.grid.labelMargin) + 'px;width:' + (xBoxWidth*4) + 'px;text-align:left;'+(axis.options.color?('color:'+axis.options.color+';'):'')+'" class="flotr-grid-label x-axis">' + tick.label + '</div>');
							}
						} else {
							html.push('<div style="position:absolute;top:0px;left:' + (this.plotOffset.left +axis.d2p(tick.v) - xBoxWidth/2) + 'px;width:' + xBoxWidth + 'px;text-align:center;'+(axis.options.color?('color:'+axis.options.color+';'):'')+'" class="flotr-grid-label x-axis">' + tick.label + '</div>');
						}
					} else {
						html.push('<div style="position:absolute;top:' + (this.plotOffset.top + this.plotHeight + options.grid.labelMargin) + 'px;left:' + (this.plotOffset.left +axis.d2p(tick.v) - xBoxWidth/2) + 'px;width:' + xBoxWidth + 'px;text-align:center;'+(axis.options.color?('color:'+axis.options.color+';'):'')+'" class="flotr-grid-label x-axis">' + tick.label + '</div>');
					}
				}
			}
			...
			if (axis.options.showLabels){
				for(i = 0; i < axis.ticks.length; ++i){
					...
					if (a.x.options.positionTop) {
						if (a.x.options.labelsAngle) {
							html.push('<div style="position:absolute;top:' + (this.plotOffset.top - options.grid.labelMargin - axis.maxLabel.height) + 'px;left:' + (this.plotOffset.left + a.x.d2p(tick.v) - xBoxWidth/2) + 'px;width:' + (xBoxWidth*2) + 'px;text-align:left;'+(axis.options.color?('color:'+axis.options.color+';'):'')+'" class="flotr-grid-label x-axis">' + tick.label + '</div>');
						} else {
							html.push('<div style="position:absolute;top:' + (this.plotOffset.top - options.grid.labelMargin - axis.maxLabel.height) + 'px;left:' + (this.plotOffset.left + a.x.d2p(tick.v) - xBoxWidth/2) + 'px;width:' + xBoxWidth + 'px;text-align:center;'+(axis.options.color?('color:'+axis.options.color+';'):'')+'" class="flotr-grid-label x-axis">' + tick.label + '</div>');
						}
					} else {
						html.push('<div style="position:absolute;top:' + (this.plotOffset.top - options.grid.labelMargin - axis.maxLabel.height) + 'px;left:' + (this.plotOffset.left + axis.d2p(tick.v) - xBoxWidth/2) + 'px;width:' + xBoxWidth + 'px;text-align:center;'+(axis.options.color?('color:'+axis.options.color+';'):'')+'" class="flotr-grid-label x-axis">' + tick.label + '</div>');
					}
				}
			}
			...
			if (axis.options.showLabels){
				for(i = 0; i < axis.ticks.length; ++i){
					...
					html.push('<div style="position:absolute;top:' + (this.plotOffset.top + axis.d2p(tick.v) - axis.maxLabel.height/2) + 'px;left:0;width:' + (this.plotOffset.left - options.grid.labelMargin) + 'px;text-align:right;'+(axis.options.color?('color:'+axis.options.color+';'):'')+'" class="flotr-grid-label y-axis">' + tick.label + '</div>');
				}
			}
	...
	drawTitles: function(){
		...
		if (!options.HtmlText && this.textEnabled) {
			...
			// Add x axis title
			if (a.x.options.title && a.x.used)
				html.push('<div style="position:absolute;top:' + (this.plotOffset.top + this.plotHeight + options.grid.labelMargin + a.x.titleSize.height) + 'px;left:' + this.plotOffset.left + 'px;width:' + this.plotWidth + 'px;text-align:center;" class="flotr-axis-title x-axis-title">' + a.x.options.title + '</div>');
			
			// Add x2 axis title
			//if (a.x2.options.title && a.x2.used)
			if (a.x2.options.title)
				html.push('<div style="position:absolute;top:0;left:' + this.plotOffset.left + 'px;width:' + this.plotWidth + 'px;text-align:center;" class="flotr-axis-title x-axis-title">' + a.x2.options.title + '</div>');
			
			// Add y axis title
			if (a.y.options.title && a.y.used)
				html.push('<div style="position:absolute;top:' + (this.plotOffset.top + this.plotHeight/2 - a.y.titleSize.height/2) + 'px;left:0;text-align:right;" class="flotr-axis-title y-axis-title">' + a.y.options.title + '</div>');
			
			// Add y2 axis title
			if (a.y2.options.title && a.y2.used)
				html.push('<div style="position:absolute;top:' + (this.plotOffset.top + this.plotHeight/2 - a.y.titleSize.height/2) + 'px;right:0;text-align:right;" class="flotr-axis-title y-axis-title">' + a.y2.options.title + '</div>');
	...
	clickHasHitDataPoint: function(clickPos) {
		...
		for(i = 0; i < series.length; i++){
			...
			for(var j = 0, xpow, ypow; j < data.length; j++){
				...
				dim = this.getTextDimensions(data[j][3], null, null);
				c = dim.height;
				//y += (dim.height - 5);
				y += ((c/2)/ya.scale);
				//y += (6/ya.scale);
				...
				//if( ((!s.barz.show) && xdiff < xsens && ydiff < ysens) 
				// || (s.barz.show && xdiff < 2/2 && ((y > 0 && clickPos.y > 0 && clickPos.y < y) || (y < 0 && clickPos.y < 0 && clickPos.y > y))) ){
				
				//if(xdiff < (xsens + (data[j][2]*xa.scale)) && ydiff < (ysens + (13*ya.scale))) {
				if ( (!s.bars.show && xdiff < xsens && ydiff < ysens) 
				 || ( s.barz.show &&
					(clickPos.x >= x && clickPos.x <= (x+data[j][2])) &&
					(clickPos.y >= y && clickPos.y <= (y+(c/ya.scale))) )
				) {
				//if(xdiff < 2/2 && ((y > 0 && clickPos.y > 0 && clickPos.y < y) || (y < 0 && clickPos.y < 0 && clickPos.y > y)) ){
					...
					if(distance < n.dist){
						...
						n.y = y;
						n.xr = (x+data[j][2]);
						n.yr = (y+(13/ya.scale)); //(y+(2*(13/ya.scale)));
						n.cx = clickPos.x;
						n.cy = clickPos.y;
						n.xaxis = xa;
						n.yaxis = ya;
	...
	mouseMoveHandler: function(event){
		...
		/*var clickPos = this.clickHasHitDataPoint(this.getEventPosition(event));
		if (clickPos != null) {
			this.el.fire('flotr:mousemoveover', [clickPos, this]);
			return;
		} else {
			var n = {
				series: this.series,
			...
			this.el.fire('flotr:mousemove', [n, this]);
		}*/
	...
	clearHit: function(){
		...
		if(!s.bars.show){
			...
			if (s.barz.show) {
				var margin = 2;
				var dim = this.getTextDimensions(prevHit.label, null, null);
				dim.width = Math.floor(dim.width+margin*2);
				dim.height = Math.floor(dim.height+margin*2);
				
				var left = xa.d2p(prevHit.x);
				var top = ya.d2p(prevHit.y);
				
				     if (this.barz.options.position.indexOf('c') != -1) left -= (prevHit.d)/2 + margin;
				else if (this.barz.options.position.indexOf('l') != -1) left -= (prevHit.d);

				     if (this.barz.options.position.indexOf('m') != -1) top -= dim.height/2 + margin;
				else if (this.barz.options.position.indexOf('t') != -1) top -= dim.height;

				left = Math.floor(left)+0.5;
				top = Math.floor(top)+0.5 + margin*2;
				
				/*xa.d2p(prevHit.x) + plotOffset.left - (lw * 2),
				ya.d2p(prevHit.y) + plotOffset.top - (lw * 2),
				xa.d2p(prevHit.x + (20*xa.scale)) + lw * 2, 
				ya.d2p(prevHit.y + 13) + lw * 2*/
				this.octx.clearRect(
					left + plotOffset.left - (lw * 2),
					top + plotOffset.top - (lw * 2),
					left + (prevHit.d*xa.scale) + lw * 2, 
					top + dim.height + lw * 2
				);
			} else {
				this.octx.clearRect(
		...
		if(s.mouse.lineColor != null){
			...
			if(!s.bars.show){
				if (s.barz.show) {
					...
					var margin = 2;
					var dim = this.getTextDimensions(n.label, null, null);
					dim.width = Math.floor(dim.width+margin*2);
					dim.height = Math.floor(dim.height+margin*2);
					...
					var top = ya.d2p(n.y);
					     ...
					     if (this.barz.options.position.indexOf('c') != -1) left -= (n.d)/2 + margin;
					else if (this.barz.options.position.indexOf('l') != -1) left -= (n.d);
					...
					octx.strokeRect(left, top, n.d*xa.scale, n.h);
					octx.fill();
					octx.stroke();
				} else {
					octx.translate(this.plotOffset.left, this.plotOffset.top);
	...
	hit: function(mouse){
		...
		for(i = 0; i < series.length; i++){
			...
			for(var j = 0, xpow, ypow; j < data.length; j++){
				...
				if (s.barz.show) {
					//dim = this.getTextDimensions(data[j][3], null, null);
					//c = dim.height;
					y += ((this.options.fontSize/2)/ya.scale);
				}
				//$('coord').value = ((this.options.fontSize/2)/ya.scale);
				if (
					(s.barz.show &&
						(mx >= x && mx <= (x+data[j][2])) &&
						(my >= y && my <= (y+(((this.options.fontSize*2)-2)/ya.scale))) //&& xdiff < xsens && ydiff < ysens
					) 
					|| ((!s.bars.show && !s.barz.show) && xdiff < xsens && ydiff < ysens) 
					...
					if(distance < n.dist){
						...
						n.x = x;
						n.y = y;
						if (s.barz.show || s.dotz.show) {
							n.d = data[j][2];
							n.h = (this.options.fontSize*2)-2;
							n.label = data[j][3];
						}
						n.xaxis = xa;
						n.yaxis = ya;
		...
		if(n.series && (n.mouse && n.mouse.track && !prevHit || (prevHit /*&& (n.x != prevHit.x || n.y != prevHit.y)*/))){
			var mt = this.mouseTrack || this.el.select(".flotr-mouse-value")[0],
			    ...
			    p = n.mouse.position, 
			    m = n.mouse.margin,
			    elStyle = 'display:none;position:absolute;';
			...
			if(!mt){
				this.el.insert('<div class="flotr-mouse-value" id="flotr-mouse-value" style="'+elStyle+'"></div>');
...
Flotr.addType('bubbles', {
	...
	draw: function(series){
		...
		ctx.strokeStyle = series.color;
		ctx.fillStyle = this.processColor(series.color, {opacity: series.bubbles.fillOpacity});
...
Flotr.addType('barz', {
	options: {
		show: false,           // => setting to true will show markers, false will hide
		lineWidth: 1,          // => line width of the rectangle around the marker
		fill: true,           // => fill or not the marekers' rectangles
		fillColor: "#00A8F0",  // => fill color
		fillOpacity: 0.4,      // => fill opacity
		fillColorHi: "#CB4B4B",  // => fill color
		stroke: true,         // => draw the rectangle around the markers
		position: 'tr',        // => the markers position (vertical align: b, m, t, horizontal align: l, c, r)
		boxHeight: 5,
		shadowSize: 1,
		showLabels: true,
		bgLevels: false,
	...
	draw: function(series){
		...
		if (taxonomy && options.bgLevels) {
			for(var i = 0; i < taxonomy.levels.length; ++i){
				...
				ctx.lineWidth = options.lineWidth;
				ctx.strokeStyle = 'rgba(0,0,0,0.5)';
				ctx.fillStyle = this.processColor(taxonomy.levels[i][4], {opacity: 0.05});
				ctx.fillRect(taxonomy.levels[i][0]*xa.scale, 0, 10*xa.scale, 90*ya.scale);
				ctx.restore();
			}
		}
		...
		for(var i = 0; i < data.length; ++i){
			...
			width = length*xa.scale;
			color = options.fillColor;
			
			if (taxonomy) {
				for(var z = 0; z < taxonomy.levels.length; ++z){
					if (options.bgLevels) {
						if (x >= taxonomy.levels[z][0] && x < taxonomy.levels[z][1]) {
							color = taxonomy.levels[z][4];
						}
					} else {
						if (data[i][6] >= taxonomy.levels[z][0] && data[i][6] < taxonomy.levels[z][1]) {
							color = taxonomy.levels[z][4];
						}
					}
				}
			...
			this.barz.plot(xPos, yPos, width, label, options, active, color);
		}
		...
		for(var i = 0; i < data.length; ++i){
			var x = data[i][0], xPos = xa.d2p(x),
			    ...
			    label = data[i][3];
			width = length*xa.scale;
			this.barz.plotShadows(xPos, yPos, width, label, options);
		}

		ctx.restore();
	},
	
	plot: function(x, y, length, label, options, active, color) {
		var ctx = this.ctx,
		    dim = this.getTextDimensions(label, null, null),
		    margin = 2,
		    left = x,
		...
		dim.height = Math.floor(dim.height+margin*2);
		     ...
		     if (options.position.indexOf('c') != -1) left -= (length)/2 + margin;
		else if (options.position.indexOf('l') != -1) left -= (length);
		...
		var width = length;
		if (active) {
			width = (dim.width > length) ? dim.width : length;
			ctx.fillStyle = this.processColor(options.fillColorHi, {opacity: options.fillOpacity});
		} else {
			ctx.fillStyle = this.processColor(color, {opacity: options.fillOpacity});
		}

		if(options.fill)
			ctx.fillRect(left, top, width, dim.height - options.boxHeight);

		if(options.stroke) 
			ctx.strokeStyle = (active) ? options.fillColorHi : color;
			ctx.lineWidth = (active) ? options.lineWidth*2 : options.lineWidth;
			ctx.strokeRect(left, top, width, dim.height - options.boxHeight);
		
		if (dim.width > length && !active) {
			var truncLength = options.truncLength;
			for(var j = 0; j < label.length; ++j){
				di = this.getTextDimensions(label.substr(0,(label.length - j)), null, null);
				if (di.width < length) {
					truncLength = label.length - j;
					break;
				}
			}
			label = label.trunc(truncLength, options.truncByWord);
		}
		
		if (options.showLabels) {
			ctx.drawText(label, left+margin, top+margin, {valign: 't', halign: 'l', color: '#000'});
		}
	},
	
	plotShadows: function(x, y, width, label, options){
		var ctx = this.ctx,
		    dim = this.getTextDimensions(label, null, null),
		    margin = 2,
		    left = x,
		    top = y;
		     if (options.position.indexOf('c') != -1) left -= (width)/2 + margin;
		else if (options.position.indexOf('l') != -1) left -= (width);
		...
		var sw = options.shadowSize;

		ctx.strokeStyle = 'rgba(0,0,0,0.05)';
		ctx.lineWidth = options.lineWidth;
		ctx.fillStyle = 'rgba(0,0,0,0.05)';

		ctx.fillRect(left+sw, top+sw, width, dim.height-options.boxHeight/2);
	}
});
...
Flotr.addType('dotz', {
	options: {
		...
		fill: true,       // => true to fill the area from the line to the x axis, false for (transparent) no fill
		fillOpacity: 0.4, // => opacity of the fill color, set to 1 for a solid fill, 0 hides the fill
		baseRadius: 3.5,     // => ratio of the radar, against the plot size
		bgLevels: false,
		extension: 'vert'
	},
	draw: function(series){
		...
		if (taxonomy && series.dotz.bgLevels) {
			for(var i = 0; i < taxonomy.levels.length; ++i){
				ctx.save();
				ctx.translate(this.plotOffset.left, this.plotOffset.top);
				ctx.lineJoin = 'round';
				ctx.lineWidth = series.dotz.lineWidth;
				ctx.strokeStyle = 'rgba(0,0,0,0.5)';
				ctx.fillStyle = this.processColor(taxonomy.levels[i][4], {opacity: 0.05});
				ctx.fillRect(taxonomy.levels[i][0]*series.xaxis.scale, 0, 10*series.xaxis.scale, 90*series.yaxis.scale);
				ctx.restore();
			}
		}
		...
		ctx.fillStyle = 'rgba(0,0,0,0.05)';
		ctx.strokeStyle = 'rgba(0,0,0,0.05)';
		this.dotz.plot(series, series.shadowSize / 2, false);
		
		// Halo shadow
		ctx.lineWidth = 0.1;
		ctx.strokeStyle = 'rgba(0,0,0,0.1)';
		this.dotz.plot(series, series.shadowSize / 4, true);
		
		// Halo
		ctx.lineWidth = 0.1;
		this.dotz.plot(series, 0, true, series.dotz.fillOpacity/2);
		
		// Dot
		ctx.lineWidth = series.dotz.lineWidth;
		this.dotz.plot(series, 0, false, series.dotz.fillOpacity);
		
		ctx.restore();
	},
	plot: function(series, offset, halo, opacity){
		var ctx = this.ctx,
		    options = this.options,
		    data = series.data,
		    radius = options.dotz.baseRadius;
		...
		for(var i = 0; i < data.length; ++i){
			...
			ctx.strokeStyle = color;
			if (opacity) {
				ctx.fillStyle = this.processColor(color, {opacity: opacity});
			}
			ctx.beginPath();
			if (halo) {
				ctx.arc(series.xaxis.d2p(x) + offset, series.yaxis.d2p(y) + offset, radius * (r/4), 0, Math.PI*2, true);
			} else {
				ctx.arc(series.xaxis.d2p(x), series.yaxis.d2p(y), radius, 0, Math.PI*2, true);
			}
			ctx.stroke();
			if (series.dotz.fill) ctx.fill();
			ctx.closePath();
			
			if (!halo && data[i][2] > 10) {
				if (series.dotz.extension == 'vert') {
					var bottom  = y - (data[i][2]/2),
			    		top     = y + (radius/4),
			    		bottom2 = y - (radius/4),
			    		top2    = y + (data[i][2]/2);

					ctx.beginPath();
					ctx.moveTo(series.xaxis.d2p(x), Math.floor(series.yaxis.d2p(top2   ) + offset));
					ctx.lineTo(series.xaxis.d2p(x), Math.floor(series.yaxis.d2p(top    ) + offset));
					ctx.moveTo(series.xaxis.d2p(x), Math.floor(series.yaxis.d2p(bottom2) + offset));
					ctx.lineTo(series.xaxis.d2p(x), Math.floor(series.yaxis.d2p(bottom ) + offset));
					ctx.stroke();
				} else {
					var bottom  = x - (data[i][2]/2),
			    		top     = x + (radius/10),
			    		bottom2 = x - (radius/10),
			    		top2    = x + (data[i][2]/2);

					ctx.beginPath();
					ctx.moveTo(Math.floor(series.xaxis.d2p(top2   ) + offset), series.yaxis.d2p(y));
					ctx.lineTo(Math.floor(series.xaxis.d2p(top    ) + offset), series.yaxis.d2p(y));
					ctx.moveTo(Math.floor(series.xaxis.d2p(bottom2) + offset), series.yaxis.d2p(y));
					ctx.lineTo(Math.floor(series.xaxis.d2p(bottom ) + offset), series.yaxis.d2p(y));
					ctx.stroke();
				}
			}
		}

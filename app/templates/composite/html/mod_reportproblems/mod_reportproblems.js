...
HUB.Modules.ReportProblems = {
	...
	initialize: function(trigger) {
		...
		$('<button class="close"><span>close</span></button>').on('click', function (e) {
			e.preventDefault();
		...
		if ($(settings.toggle).length) {
			$(settings.toggle).on('click', function (e) {
				...
				if (!(panel.hasClass('open'))) {
					...
					if (typeof UI != 'undefined') {
						if ($('#session').length) {
							// If a session is running, we need to push it out of the way
							// as it can interfere with click events.
							$('#session').css('margin-left', '-9999px');
						}

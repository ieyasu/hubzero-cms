...
$(document).ready(function() {
	...
	HUB.template.homeUpdateCarousels = function() {
		$('.carousel .tools').slick('setPosition');
		$('.carousel .resources').slick('setPosition');

		var tools = $('.carousel .tools .tool .contents');
		tools.attr('style', '');
		var maxH = 0;

		$.each(tools, function (k, v) {
			if ($(v).outerHeight() > maxH) {
				maxH = $(v).outerHeight();
			}
		});
		tools.outerHeight(maxH);

		var resources = $('.carousel .resources .resource .contents');
		...
		resources.outerHeight(maxH);
	};

	HUB.template.homeCarousels = function() {
		var toolsCarousel = $('.carousel .tools');
